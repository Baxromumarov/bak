...
func parserErrorToDiagnostic(message, source string) (Diagnostic, bool) {
	matches := lineColRegex.FindStringSubmatch(message)
	if len(matches) != 4 {
	...
	return Diagnostic{
		...
		Message:  body,
		Data:     parserDiagnosticData(body, source, line),
	}, true
}
...
func typeErrorToDiagnostic(typeErr typechecker.TypeError) Diagnostic {
	severity := 1
