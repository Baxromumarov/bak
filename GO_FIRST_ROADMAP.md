...
13. A third native smoke matrix now covers imported `std/time` and `std/strings` programs.
14. A focused native regression test now covers deterministic time builtins and binary execution.
15. A CI workflow now covers formatter, linter, parser, typechecker, compiler, VM, native, builtins, and runtimecap package tests.
...
6. A third native smoke matrix now covers imported `std/time` and `std/strings` programs.
7. A focused native regression test now covers deterministic time builtins and binary execution.
8. `runtimecap.ExecuteCommand` now has direct tests and stable timeout exit codes.
9. Native backend regression coverage now also pins fs-mutate and network permission gates on both allow and deny sides.
10. Shared `.bak` file collection was extracted for `bakfmt`, `bakcheck`, and `baklint`.
11. Filesystem mutation helpers now enforce permission gates and reject obviously unsafe targets.
12. `bakc-test` now also uses the shared `.bak` file collector.
13. Native backend smoke coverage now includes file predicate builtins, the std/fs remove and mkdir paths, and a native `std/os.chdir` regression that fixed `os.cwd` Result tagging (`__builtin_file_exists`, `__builtin_is_file`, `__builtin_is_dir`, `__builtin_remove`, `__builtin_mkdir`, `__builtin_chdir`).
14. A direct native regression test now covers `os.getenv` positive lookup under a known environment sentinel.
15. `go test ./...` was kept green after each landed tranche.
